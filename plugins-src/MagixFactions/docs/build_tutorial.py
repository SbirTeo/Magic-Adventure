...
<div class="toc">
  ...
  <ol>
    ...
    <li><a href="#s8">La mappa (/f map)</a></li>
    <li><a href="#s9">La casa (/f home)</a></li>
    <li><a href="#s10">Perdere territori (sovraccarico)</a></li>
    <li><a href="#s11">Guardare le info</a></li>
    <li><a href="#sTop">La classifica del server</a></li>
    <li><a href="#s12">Tutti i comandi</a></li>
  </ol>
</div>
...
<section id="s7">
  ...
  <h2><span class="n">B</span>La Banca della fazione</h2>
...
<section id="s8">
  <h2><span class="n">8</span>La Mappa Fazioni — <span class="cmd" style="font-size:15px">/f map</span></h2>
{{se:map.mode=chat}}
  <p>Il comando <span class="cmd">/f map</span> ti stampa <b>in chat</b> la mappa dei territori attorno a te: un
  quadrato di <b>{{cfg:map.chat.rows}}×{{cfg:map.chat.rows}}</b> caselle con <b>te al centro</b>. Ogni casella è un
  pezzo di terreno (un <i>chunk</i>, 16×16 blocchi). Se appartiene a una fazione ci trovi una <b>lettera</b>,
  colorata secondo la tua <b>relazione</b> con chi lo possiede — <b>verde</b> = la tua fazione, <b>viola</b> =
  alleata, <b>rossa</b> = nemica — e sotto la mappa la <b>legenda</b> ti dice di quale fazione è ogni lettera.</p>
  <div class="chat">
  ...
  in un angolo, quello sì che si aggiorna da solo mentre cammini — è invece un extra riservato ai
  <b>VIP</b>.</div>
{{/se}}
{{se:map.mode=item}}
...
<section id="s9">
  <h2><span class="n">9</span>La casa della fazione — <span class="cmd" style="font-size:15px">/f home</span></h2>
  <ol class="steps">
...
<section id="s10">
  <h2><span class="n">10</span>Perdere territori (sovraccarico)</h2>
...
<section id="s11">
  <h2><span class="n">11</span>Guardare le info</h2>
...
<section id="sTop">
  ...
  e basta, o più soldi e basta: ogni fazione ha un <b>Punteggio</b> da zero a cento che <b>mette insieme</b>
  più cose, ognuna che pesa quanto lo staff decide.</p>
  <div class="chat">
    <span class="g">— Classifica fazioni (4) —</span><br>
    <span class="gr">#1</span> <span class="gn">Draghi</span> <span class="gr">-</span> <span class="cy">72,5</span><br>
    <span class="gr">#2</span> <span class="rd">Corvi</span> <span class="gr">-</span> <span class="cy">64,1</span><br>
    <span class="gr">#3</span> <span class="rd">Lupi</span> <span class="gr">-</span> <span class="cy">51,8</span><br>
    <span class="gr">#4</span> <span class="rd">Volpi</span> <span class="gr">-</span> <span class="cy">39,0</span>
  </div>
  <p>Le cose che fanno salire il Punteggio sono cinque:</p>
  <ul>
    <li><b>Territori</b> — quanti chunk possiede la fazione (peso {{cfg:score.weights.land}}).</li>
    <li><b>Membri</b> — quanti siete (peso {{cfg:score.weights.members}}).</li>
    <li><b>Banca</b> — la <b>giacenza media</b>, cioè quanti soldi tenete <b>nel tempo</b> (peso {{cfg:score.weights.bank}}).</li>
    <li><b>Longevità</b> — da quanti giorni esiste la fazione (peso {{cfg:score.weights.longevity}}).</li>
    <li><b>Potenza</b> — la potenza media della fazione (peso {{cfg:score.weights.power}}).</li>
  </ul>
  <p class="sub">Due cose importanti. La <b>banca</b> conta la <b>media nel tempo</b>: mettere un milione un
  attimo prima di guardare la classifica <b>non serve</b> a scalarla — contano i soldi che tieni davvero.
  E ogni voce ha un <b>tetto morbido</b>: strafare in una sola cosa rende sempre un po' di più, ma sempre
  meno — per andare in alto conviene <b>crescere su tutto</b>.</p>
  <p class="sub">Il tuo Punteggio e la tua posizione li vedi anche in <span class="cmd">/f info</span> e sul
  <b>sito</b>.</p>
</section>

<section id="s12">
  <h2><span class="n">12</span>Tutti i comandi</h2>
  <table>
