 ...
 *   <li>se si', il provvedimento passa dalla {@link Politica}, che decide se applicarlo o
...
public final class Rilevatore {
    ...
    public Rilevatore(JavaPlugin plugin, SanzioniConfig cfg, ServizioSanzioni servizio,
                      ViolazioniDao violazioni, RegistroPunti registro) {
        this.plugin = plugin;
        this.cfg = cfg;
    ...
    public void rileva(UUID uuid, String nome, String categoria, String fonte, String dettaglio,
        ...
        SanzioniConfig.Categoria cat = cfg.categoria(categoria);
        ...
        Bukkit.getScheduler().runTaskAsynchronously(plugin, () -> {
            try {
                ...
                int idViolazione = violazioni.inserisci(
                        Violazione.nuova(uuid, nome, categoria, punti, fonte, dettaglio));
                double dopo = prima + punti;

                SanzioniConfig.Soglia soglia = sogliaAppenaSuperata(prima, dopo);
                if (soglia == null) {
                ...
                long fine = soglia.durata() == Durata.PERMANENTE || !soglia.tipo().haDurata()
                        ? Durata.PERMANENTE : adesso + soglia.durata();

                String motivo = cat.nome() + " (" + Math.round(dopo) + " punti)";
                Sanzione s = new Sanzione(0, uuid, nome, soglia.tipo(), categoria, motivo,
                        cat.ambito(), 0, adesso, fine, null, true, null);

                Politica.Esito esito = servizio.politica()
     ...
     */
    private SanzioniConfig.Soglia sogliaAppenaSuperata(double prima, double dopo) {
        SanzioniConfig.Soglia trovata = null;
        for (SanzioniConfig.Soglia s : cfg.soglie) {   // ordinate dalla piu' alta
     ...
     */
    private void avvisaSeServe(SanzioniConfig.Categoria cat, String nome, String categoria,
    ...
    private String componiDettaglio(String dettaglio, double prima, double dopo,
        ...
        b.append("Provvedimento previsto: ").append(soglia.tipo().etichetta())
         .append(", ").append(Durata.scrivi(soglia.durata())).append("\n\n");
        if (dettaglio != null) {
            b.append(dettaglio);
        }
        return b.toString();
