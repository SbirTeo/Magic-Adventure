 ...
 */
public final class ComandoViolazione implements CommandExecutor {

    private final JavaPlugin plugin;
    private final Rilevatore rilevatore;
    private final SanzioniDao dao;

    public ComandoViolazione(JavaPlugin plugin, Rilevatore rilevatore, SanzioniDao dao) {
        this.plugin = plugin;
    ...
    public boolean onCommand(CommandSender chi, Command comando, String etichetta, String[] args) {
        if (args.length < 2) {
            chi.sendMessage(Testo.msg("&#FFD166Uso: &f/mgviolazione <giocatore> <categoria> [dettaglio]"));
            chi.sendMessage(Testo.panel("&7Lo chiama l'anticheat, non una persona. Categorie: "
