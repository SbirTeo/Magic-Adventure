...
public final class Politica {
    ...
    public Politica(SanzioniConfig cfg) {
        this.cfg = cfg;
    }
    ...
    public Esito controllaStaff(CommandSender chi, Tipo tipo, long durata) {
        if (!(chi instanceof Player)) {
            return Esito.si();
        }
        String grado = grado(chi);
        SanzioniConfig.Tetto tetto = cfg.tetto(grado);
        if (tetto.consente(tipo, durata)) {
            return Esito.si();
        }
        String limite = tipo == Tipo.BAN ? Durata.scrivi(tetto.ban()) : Durata.scrivi(tetto.mute());
     ...
     */
    public Esito controllaAutomatismo(SanzioniConfig.Categoria categoria, Tipo tipo, long durata) {
        if (tipo == Tipo.BAN && durata == Durata.PERMANENTE) {
    ...
    private Esito dentroIlTetto(long durata) {
        ...
        if (durata == Durata.PERMANENTE || durata > cfg.durataMassimaAutomatica) {
            return Esito.proponi("supera il tetto automatico di "
                    + Durata.scrivi(cfg.durataMassimaAutomatica));
        }
        return Esito.si();
    }
