 ...
 */
public final class SanzioniListener implements Listener {
    ...
    public SanzioniListener(JavaPlugin plugin, SanzioniConfig cfg, ServizioSanzioni servizio, SanzioniDao dao) {
        this.plugin = plugin;
        this.cfg = cfg;
    ...
    public void suPreLogin(AsyncPlayerPreLoginEvent e) {
        try {
            List<Sanzione> attive = dao.attiveInGioco(e.getUniqueId());
            for (Sanzione s : attive) {
                if (s.tipo() == Tipo.BAN) {
                    e.disallow(AsyncPlayerPreLoginEvent.Result.KICK_BANNED,
                            Testo.c(servizio.messaggioBan(s)));
                    return;
                }
            }
    ...
    public void suChat(AsyncChatEvent e) {
        Sanzione s = servizio.muto(e.getPlayer().getUniqueId());
        if (s == null) {
            return;
        }
        e.setCancelled(true);
        String scadenza = s.fine() == Durata.PERMANENTE ? "non scade" : Durata.mancante(s.fine());
        e.getPlayer().sendMessage(Testo.msg(Testo.sostituisci(
