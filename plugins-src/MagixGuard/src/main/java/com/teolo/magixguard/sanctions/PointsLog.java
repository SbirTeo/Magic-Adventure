...
public final class PointsLog {
    ...
    public PointsLog(ViolationsDao violazioni, SanctionsConfig cfg) {
    ...
    public double punti(List<Violation> storico, long adesso) {
        double totale = 0;
        for (Violation v : storico) {
     ...
     */
    public SanctionsConfig.Soglia soglia(double punti) {
        for (SanctionsConfig.Soglia s : cfg.soglie) {   // gia' ordinate dalla piu' alta
            if (punti >= s.punti()) {
                return s;
            }
        }
    ...
    public double mancanteAllaProssima(double punti) {
        double prossima = -1;
        for (SanctionsConfig.Soglia s : cfg.soglie) {
