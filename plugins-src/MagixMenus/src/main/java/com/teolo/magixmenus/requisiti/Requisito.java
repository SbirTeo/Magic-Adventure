 ...
 */
public record Requisito(Tipo tipo, String chiave, String valore, int quantita, boolean uguale) {
    ...
    public enum Tipo {
        ...
        PERMESSO,
        /** La condizione scritta in "chiave" e' vera: "%vault_eco_balance% >= 1000". Vedi {@link Equazione}. */
        EQUAZIONE,
    ...
    private boolean vero(Player p, Map<String, String> variabili) {
        String k = Testo.grezzo(p, variabili, chiave);
        String v = Testo.grezzo(p, variabili, valore);
        ...
        return switch (tipo) {
            ...
            case EQUAZIONE -> {
                try {
                    yield Equazione.vera(k);
                } catch (Equazione.Errore e) {
            ...
            case HA_SOLDI -> Economia.disponibile() && Economia.saldo(p) >= quantita;
