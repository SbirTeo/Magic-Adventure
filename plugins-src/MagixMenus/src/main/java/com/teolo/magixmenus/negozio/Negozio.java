package com.teolo.magixmenus.negozio;
...
public final class Negozio {
     ...
     */
    public static Esito clic(MagixMenus plugin, Player p, ItemDef def, Clic tasto,
        ...
        boolean vuoleVendere = vende && (tasto == Clic.DESTRO || tasto == Clic.SHIFT_DESTRO);
        if (vuoleVendere) {
    ...
    private static Esito compra(MagixMenus plugin, Player p, ItemDef def, Map<String, String> variabili) {
        ...
        if (prezzo > 0 && !Economia.togli(p, prezzo)) {
        ...
        plugin.messaggi().send(p, "shop.bought",
                ...
                "balance", numero(Economia.saldo(p)));
    ...
    public static ItemStack merce(MagixMenus plugin, Player p, Map<String, String> variabili, ItemDef def) {
        ...
        String risolto = Testo.grezzo(p, variabili, cosa).trim();
        if (risolto.equalsIgnoreCase("self") || risolto.equalsIgnoreCase("se_stesso")
                || risolto.equalsIgnoreCase("questo")) {
            ItemStack copia = CostruttoreItem.costruisci(plugin, p, variabili, def, false);
    ...
    private static double quanto(Player p, Map<String, String> variabili, String scritto) {
        Double n = Testo.numero(Testo.grezzo(p, variabili, scritto));
