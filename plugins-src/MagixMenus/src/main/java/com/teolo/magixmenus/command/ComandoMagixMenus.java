...
import com.teolo.magixmenus.util.Aiuto;
 ...
 *
 * I menu veri hanno ognuno il proprio comando (vedi {@link ComandoMenu}); questo serve a chi li
 * amministra — ricaricare, vedere quali ci sono, aprirne uno a qualcun altro, capire perche' uno
 * non funziona.
 */
public final class ComandoMagixMenus implements CommandExecutor, TabCompleter {

    private final MagixMenus plugin;

    public ComandoMagixMenus(MagixMenus plugin) {
        this.plugin = plugin;
    }
    ...
    public boolean onCommand(CommandSender m, Command c, String etichetta, String[] a) {
        ...
        if (a.length == 0 || a[0].equalsIgnoreCase("help") || a[0].equalsIgnoreCase("aiuto")) {
            ...
            Aiuto.mostra(m, "MagixMenus", "/" + etichetta + " help",
                    Aiuto.daConfig(plugin.messaggi().section("help.sections")), pagina, staff);
            return true;
        }
