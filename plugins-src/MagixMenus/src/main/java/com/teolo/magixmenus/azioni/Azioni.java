package com.teolo.magixmenus.azioni;
...
public final class Azioni {
    ...
    public static void esegui(MagixMenus plugin, Contesto contesto, List<Azione> azioni) {
        esegui(plugin, contesto, azioni, 0);
    }

    private static void esegui(MagixMenus plugin, Contesto contesto, List<Azione> azioni, int da) {
        Player p = contesto.giocatore();
        for (int i = da; i < azioni.size(); i++) {
            Azione a = azioni.get(i);

            if (a.tipo() == Azione.Tipo.SE) {
                List<Azione> ramo = a.condizione().soddisfatti(p, contesto.variabili())
                        ? a.allora() : a.altrimenti();
                esegui(plugin, contesto, ramo, 0);
                continue;
            }

            if (a.tipo() == Azione.Tipo.ATTESA) {
    ...
    private static boolean una(MagixMenus plugin, Contesto contesto, Azione a) {
        Player p = contesto.giocatore();
        String arg = Testo.grezzo(p, contesto.variabili(), a.argomento());
        ...
        switch (a.tipo()) {
            ...
            case TOGLI_SOLDI -> {
                ...
                if (!Economia.togli(p, quanto(arg))) {
                    return false;
                }
            }
