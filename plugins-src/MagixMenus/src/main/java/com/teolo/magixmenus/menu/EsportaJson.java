 ...
 * significa che chi legge {@code EsportaJson} accanto a {@code ItemDef} vede le stesse parole.
 ...
 */
public final class EsportaJson {

    private EsportaJson() {
    }

    public static void scrivi(JavaPlugin plugin, GestoreMenu gestore) {
    ...
    private static JsonObject item(ItemDef i, boolean conCaselle) {
        ...
        // riscrive lo YAML dal sito non deve tradurre niente, e un nome nuovo aggiunto a Clic
    ...
    private static JsonObject dialogo(Dialogo d) {
        ...
        for (Dialogo.Bottone b : d.bottoni()) {
            JsonObject x = new JsonObject();
    ...
    private static JsonObject requisiti(Requisiti r) {
        JsonObject o = new JsonObject();
        o.addProperty("minimo", r.minimo());
        JsonArray elenco = new JsonArray();
        for (Requisito x : r.elenco()) {
    ...
    private static JsonArray azioni(List<Azione> elenco) {
        JsonArray a = new JsonArray();
        for (Azione x : elenco) {
            JsonObject o = new JsonObject();
            o.addProperty("tipo", x.tipo().nomeFile());
            if (x.tipo() == Azione.Tipo.SE) {
    ...
    private static JsonObject catalogo() {
        JsonObject o = new JsonObject();
        o.add("tipi_menu", testi(TipoMenu.nomi()));
        o.add("tipi_requisito", testi(Requisito.tipiDisponibili()));
        o.add("tipi_azione", testi(Azione.tipiDisponibili()));
