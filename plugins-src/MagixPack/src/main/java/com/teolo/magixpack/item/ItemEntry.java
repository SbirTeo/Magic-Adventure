 ...
 *
 * <p>{@code furniture}: se true, l'oggetto si puo' piazzare per terra (tasto destro dolente +
 * shift su un blocco, vedi {@code furniture.FurnitureListener}) invece di restare solo in mano/
 * inventario — di default false, va scelto voce per voce in items.yml. {@code furnitureSolid}
 * conta solo quando furniture e' true: se true, il posto occupato blocca davvero il movimento
 * (un blocco LIGHT invisibile di livello 0), altrimenti resta attraversabile — scelta per
 * oggetto, non globale.
 */
public record ItemEntry(String id, String material, String name, List<String> lore, int customModelData,
                         boolean furniture, boolean furnitureSolid) {
}
